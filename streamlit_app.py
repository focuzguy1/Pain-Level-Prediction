...
.metric-card-green {
    background: #eafaf1; padding: .65rem 1rem; border-radius: 8px;
    border-left: 4px solid #27ae60; margin: .35rem 0; font-size: .88rem;
}
.metric-card-orange {
    background: #fef9e7; padding: .65rem 1rem; border-radius: 8px;
    border-left: 4px solid #f39c12; margin: .35rem 0; font-size: .88rem;
}
.section-hdr {
...
def main():
    ...
    with st.sidebar:
        ...
        <div class="metric-card-green"><b>Sensitivity (macro):</b> {bundle.get('loo_macro_sensitivity', 0)*100:.1f}%</div>
        <div class="metric-card-green"><b>Specificity (macro):</b> {bundle.get('loo_macro_specificity', 0)*100:.1f}%</div>
        ...
        if st.button("🔍  Predict Pain Level", use_container_width=True):
            with st.spinner("Running prediction..."):
                try:
                    ...
                    # ── Row 2: Sensitivity & Specificity ─────
                    st.markdown("### 📐 Sensitivity & Specificity (LOO-CV)")
                    st.caption(
                        "Computed per class using one-vs-rest from the confusion matrix. "
                        "These are fixed training metrics — not per-patient values."
                    )

                    per_cls_ss = bundle.get('loo_per_class_sens_spec', {})

                    # Highlighted metrics for predicted class
                    if per_cls_ss:
                        cls_ss = per_cls_ss.get(pred, per_cls_ss.get(str(pred), {}))
                        if cls_ss:
                            m1, m2, m3 = st.columns(3)
                            with m1:
                                st.metric(
                                    label=f"Sensitivity — {PAIN_LABELS[pred]}",
                                    value=f"{cls_ss.get('sensitivity', 0)*100:.1f}%",
                                    help="Of all patients truly in this class, % the model correctly identified"
                                )
                            with m2:
                                st.metric(
                                    label=f"Specificity — {PAIN_LABELS[pred]}",
                                    value=f"{cls_ss.get('specificity', 0)*100:.1f}%",
                                    help="Of all patients NOT in this class, % the model correctly excluded"
                                )
                            with m3:
                                st.metric(
                                    label="Model confidence",
                                    value=f"{proba.get(pred, 0)*100:.1f}%",
                                    help="Predicted probability for this class"
                                )

                        # Full per-class table
                        st.markdown("**All classes — sensitivity & specificity**")
                        ss_rows = []
                        for cls in [1, 2, 3, 4]:
                            cd = per_cls_ss.get(cls, per_cls_ss.get(str(cls), {}))
                            mark = " ◀ predicted" if cls == pred else ""
                            ss_rows.append({
                                'Pain Level'  : PAIN_LABELS[cls] + mark,
                                'Sensitivity' : f"{cd.get('sensitivity', 0)*100:.1f}%",
                                'Specificity' : f"{cd.get('specificity', 0)*100:.1f}%",
                                'TP' : cd.get('TP', '—'),
                                'FP' : cd.get('FP', '—'),
                                'TN' : cd.get('TN', '—'),
                                'FN' : cd.get('FN', '—'),
